...
import { validateAttachments } from '../lib/email';
...
async function testAccountLockout() {
  ...
  async function simulateLoginFailure() {
    ...
    if (user.lockedUntil && user.lockedUntil > now) {
      return { error: "Locked out", shouldLock: true };
    }
  ...
  if (!updated5) throw new Error("User not found");
  ...
  if (!updatedReset) throw new Error("User not found");
